...
export class PromptTuner {
  ...
  tunePrompt(
    ...
    const currentF1 = failurePatterns.length > 0 ? Math.max(0.3, 0.7 - failurePatterns.length * 0.05) : 0.6;
  ...
  tuneAllSkills(
    ...
    for (const [skill, originalPrompt] of Object.entries(originalPrompts)) {
      ...
      if (analysis.needsTuning) {
        ...
        // Use actual currentF1 from analysis if available
        if (analysis.currentF1 > 0) {
          tuningResult.improvement.beforeF1 = analysis.currentF1;
          tuningResult.improvement.predictedAfterF1 = estimateF1Improvement(
            analysis.currentF1,
            analyzeFailurePatterns(analysis.failurePatterns).dominantMode,
            tuningResult.changes.length,
          );
        }
