...
<head>
  ...
  <title>QA-Agent Report - ${e(report.project.name)}</title>
  <style>
...
<body>
  <div class="container">
    <div class="header">
      ...
      <div class="meta">
        <span>项目: ${e(report.project.name)}</span> ·
        <span>时间: ${e(new Date(report.timestamp).toLocaleString('zh-CN'))}</span> ·
    ...
    <div class="score-ring">
      <div class="inner">
        ...
        <div class="grade">Grade ${e(report.summary.grade)}</div>
      </div>
    </div>
    ...
    <div class="issues">
      ...
      <div class="issue ${e(issue.severity)}">
        <h4>${e(issue.id)}: ${e(issue.title)}</h4>
        <div class="location">📍 ${e(issue.location.file)}${issue.location.line ? `:${issue.location.line}` : ''}</div>
        <div class="description">${e(issue.description)}</div>
      </div>
      `).join('')}
    ...
    <div class="footer">
      Generated by QA-Agent v${e(report.version)}
    </div>
  </div>
</body>
</html>`;
