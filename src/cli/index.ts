...
program
  ...
  .option('-t, --task <task>', 'Natural-language task to execute')
  .option('-n, --name <name>', 'Visual test name')
  .option('-f, --file <file>', 'Recording file path (for --play/--export)')
  .option('--format <format>', 'Export format: json, yaml, playwright', 'playwright')
  .option('--baseline-dir <dir>', 'Visual baseline directory')
  .option('--current-dir <dir>', 'Visual current directory')
  .option('--threshold <pct>', 'Visual diff threshold percentage')
  .option('--selectors <list>', 'Comma-separated selectors for visual test')
  .option('--slow-mo <ms>', 'Playback slow motion in ms')
  .option('--retry <count>', 'Retry count for playback', '3')
  .option('--output <dir>', 'Output directory for recordings')
  .option('--interactive', 'Interactive recording mode')
  .option('--report', 'Generate test report')
  .option('--timeout <ms>', 'Browser timeout in ms')
  ...
  .option('--list', 'List recordings')
  .option('--export', 'Export a recording to a different format')
  .action(guiCommand);
